...
from .serializers import ClientsSerializer, ClientsSerializerID,DataGridSerializer
...
class ClientsView (APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"Response": "Clients Data is Successfully saved", "data": serializer.data},status=status.HTTP_201_CREATED)
    ...
    def put(self, request, Client_ID, *args, **kwargs):
        ...
        try:
            ...
            if serializer.is_valid():
                serializer.save()
                return Response({"Response": "Clients Data is Successfully Updated", "data": serializer.data},status=status.HTTP_200_OK)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        except Clients.DoesNotExist:
            return Response({"Response": "Clients Data Not Found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def delete(self, request, Client_ID, *args, **kwargs):
        try:
            ClientsData = Clients.objects.get(Client_ID=Client_ID)
            ClientsData.delete()
            return Response({"Response": "Clients Data is Successfully Deleted"}, status=status.HTTP_204_NO_CONTENT)
        except Clients.DoesNotExist:
            return Response({"Response": "Clients Data Not Found"}, status=status.HTTP_404_NOT_FOUND)
